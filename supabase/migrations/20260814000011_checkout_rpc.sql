...
--
-- The browser sends menu ids and quantities. NOTHING ELSE IS TRUSTED — every
-- price, fee, discount and total is derived here, inside the database, from
-- the authoritative menu_items rows. A manipulated client can change what it
-- asks for, never what it pays.
--
-- RLS on orders/order_items/order_status_history denies INSERT to customers
-- outright, so this SECURITY DEFINER function is the ONLY path by which a
-- customer order can come into existence.
--
-- Because it is SECURITY DEFINER it runs with the owner's rights and bypasses
-- RLS, so every statement below is explicitly scoped to auth.uid().
-- ---------------------------------------------------------------------------

alter table public.orders
  add column if not exists idempotency_key text;

-- One order per (customer, idempotency key). This is what makes a double
-- submit — double click, retry, flaky connection — return the FIRST order
...
volatile
as $$
  select 'KD-' || to_char(now() at time zone 'Africa/Lagos', 'YYMMDD')
