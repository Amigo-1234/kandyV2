...
4. **Reviews** — `reviews/{uid_menuId}` exists in V1 and the rules allow it,
   but there is no review UI yet. The food card already renders a rating if
   one is present, so this is additive.
